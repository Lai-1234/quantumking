...
- `CStrategy_ADX_Trend.mqh` is v1.12. ADX has 4 exposed inputs; H4 EMA 50/200, London/NY session filter, TP=10000, trail-off, and rising-ADX filter are locked internally.
...
| `CStrategy_ADX_Trend.mqh` (v1.12) | ADX strategy with 4 exposed inputs plus locked H4 EMA 50/200, London/NY session, rising-ADX, TP=10000, and trail-off internals. Disabled unless selected by preset/custom mode. |
...
2. **ADX_Trend v1.12 is implemented.** Use `QK_PRESET_ADX_ONLY` and optimize only the 4 remaining ADX inputs unless new OOS evidence says otherwise.
